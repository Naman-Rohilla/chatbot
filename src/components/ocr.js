  ...
  const handleUpload = async () => {
    ...
    if (file) {
      ...
      try {
        ...
        const pageHeight = 297; // Height of the page in mm (A4 size)
        ...
        lines.forEach((line, index) => {
          if (y + lineHeight > pageHeight - margin) {
            // If the current line will overflow to the next page, add a new page
            doc.addPage();
            y = margin;
          }
    ...
    <>
      <div className="h-screen bg-gray-800 flex relative overflow-y-clip ocrweb">
        ...
        <div className="inner-ring_2 z-1 shadow-inner shadow-black relative">
          <img
            className="h-40 skew-y-10 skew-x- 10 absolute bottom-32 -left-10 z-10"
            src="./boy1.png"
          ></img>
        </div>
        {/* <h1>Upload Images for OCR</h1>
        
        <button onClick={handleUpload}>Process Images</button> */}
        <input
          type="file"
          accept=".pdf"
        ...
        <div className=" w-full flex justify-center items-center">
          {file == null ? (
            <div
              ...
              className="group ocrcontainer cursor-pointer flex justify-center items-center relative"
            >
              <span className="bg-gray-800 flex justify-center cursor-pointer items-center ocrbox border border-dotted border-white shadow-gray-900 shadow-xl group-hover:bg-gray-900 group-hover:border-gray-300"></span>
              <span className="group-hover:text-gray-200 absolute text-9xl pb-20 text-gray-400">
                +
              </span>
            ...
            >
              ...
              <span className="group-hover:text-gray-200  pb-10 text-gray-400">
                {file.name}
              </span>
              {!loading ? (
                <span
                  onClick={handleUpload}
                  className="mt-1 bg-gray-50 hover:scale-105 ease-in-out duration-100 hover:bg-white w-28 h-9 flex justify-center items-center"
                >
                  <>Process PDF</>
                </span>
              ) : (
